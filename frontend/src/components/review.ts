...
import type { PendingResponse, PendingFact } from '../types.js';

export async function loadPending(): Promise<void> {
    const container = document.getElementById('pendingContainer') as HTMLElement;
    ...
    try {
        ...
        data.pending.forEach((p: PendingFact) => {
        ...
        errDiv.textContent = 'Failed to load review queue: ' + (error as Error).message;
...
async function approvePending(id: string): Promise<void> {
    try {
        ...
        showToast('Approve failed: ' + (error as Error).message, 'error');
    }
}
...
async function rejectPending(id: string): Promise<void> {
    try {
        ...
        showToast('Reject failed: ' + (error as Error).message, 'error');
    }
}
...
export async function approveAllPending(): Promise<void> {
    try {
        ...
        showToast('Approve all failed: ' + (error as Error).message, 'error');
    }
}

function editApprovePending(id: string, currentContent: string): void {
    showPrompt('Edit fact content before approving:', currentContent, (newContent) => {
        ...
        .catch((err: Error) => showToast('Edit-approve failed: ' + err.message, 'error'));
    });
}
...
export async function rejectAllPending(): Promise<void> {
    showConfirm('Reject all pending facts? This cannot be undone.', async () => {
        try {
            ...
            showToast('Reject all failed: ' + (error as Error).message, 'error');
        }
