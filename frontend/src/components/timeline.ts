...
import type { TimelineData, TimelineFact } from '../types.js';

export async function loadTimeline(): Promise<void> {
    const container = document.getElementById('timelineContainer') as HTMLElement;
    const statsDiv = document.getElementById('timelineStats') as HTMLElement;
    ...
    try {
        ...
        data.dates.forEach(dateKey => {
            ...
            group.forEach((fact: TimelineFact) => {
        ...
        errDiv.textContent = 'Failed to load timeline: ' + (error as Error).message;
